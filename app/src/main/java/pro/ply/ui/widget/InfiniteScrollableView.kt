...
class InfiniteScrollableView(context: Context, attrs: AttributeSet) :
    ...
    private fun scrollHorizontal(dx: Int) {
        ...
        if (offset[0] > offsetRightLimit) {
            ...
            val moveChild = getChildAt(moveChildIndex)
            ...
            if (moveChild is CellViewDataSetter) {
                moveChild.setData(firstVisibleChild)
            }
    ...
    private fun onClickCell(rawX: Int, rawY: Int) {
        ...
        for (i in 0 until childCount) {
            ...
            if (rect.contains(rawX, rawY) && child is OnClickCellListener) {
    ...
    fun interface CellViewDataSetter {
        fun setData(firstVisibleView: View)
    }
