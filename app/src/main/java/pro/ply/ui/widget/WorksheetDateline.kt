...
class WorksheetDateline(context: Context, attrs: AttributeSet) :
     ...
     * must be called
     */
    fun initial(initialTimestamp: Long) {
        for (i in 0 until childCount) {
            val child = getChildAt(i) as WorksheetDatelineCell
            val timestamp = initialTimestamp + i
            child.setDate(timestamp)
        }
    }
    ...
    private fun scrollHorizontal(dx: Int) {
        ...
        if (offset[0] > offsetRightLimit) {
            ...
            val moveChild = getChildAt(moveChildIndex) as WorksheetDatelineCell
